...
SELECT 
    ...
    INITCAP(STORAGE_CONDITION)::VARCHAR(255) AS STORAGE_CONDITION,
    ...
    CASE 
        ...
        ELSE 'Unknown'
    END::VARCHAR(255) AS INVENTORY_STATUS,
    ...
    GREATEST(QUANTITY_IN_STOCK, 0)::NUMBER(38,0) AS QUANTITY_IN_STOCK,

    -- Convert RATING to FLOAT with two decimal places
    ROUND(TRY_CAST(REPLACE(RATING, ',', '.') AS FLOAT), 2)::FLOAT AS RATING,

    SALES_VOLUME::NUMBER(38,0) AS SALES_VOLUME,

    -- Convert WEIGHT to FLOAT for calculations
    ROUND(TRY_CAST(REPLACE(WEIGHT, ',', '.') AS FLOAT), 2)::FLOAT AS WEIGHT_KG,

    -- Convert DISCOUNTS to FLOAT for calculations
    ROUND(TRY_CAST(REPLACE(DISCOUNTS, ',', '.') AS FLOAT), 2)::FLOAT AS DISCOUNT_PERCENTAGE,
    ...
    DATEDIFF(DAY, LAST_RESTOCK_DATE, CURRENT_DATE)::NUMBER(38,0) AS DAYS_SINCE_RESTOCK,

    -- Predict next stock depletion date (Avoid division by zero)
    DATEADD(DAY, GREATEST(QUANTITY_IN_STOCK, 0) / NULLIF(AVERAGE_MONTHLY_DEMAND, 0) * 30, CURRENT_DATE)::DATE AS ESTIMATED_STOCK_OUT_DATE,

    -- Flag critical stock conditions
    CASE 
        WHEN GREATEST(QUANTITY_IN_STOCK, 0) < REORDER_LEVEL THEN 'Low Stock'
        ELSE 'Sufficient Stock'
    END::VARCHAR(255) AS STOCK_STATUS,
    ...
    CASE 
        ...
        ELSE 'Normal' 
    END::VARCHAR(255) AS RESTOCK_PRIORITY,

    -- Add updated_at column for incremental logic
    CURRENT_TIMESTAMP::TIMESTAMP_NTZ(9) AS updated_at   -- Assuming the current timestamp for new records
